...
for b in f2:
    with open("001001单选/" + b, 'r', encoding="utf-8", errors="ignore") as load_f:
        ...
        for d in data:
            q = {'id': d['id'], 'answer': d['answer'],
                 'A': d['choices'][0]['content'], 'B': d['choices'][1]['content'], 'C': d['choices'][2]['content'], 'D': d['choices'][3]['content']}
            data_json = json.dumps(q, ensure_ascii=False)
...
for b in f2:
    with open("001002多选/" + b, 'r', encoding="utf-8", errors="ignore") as load_f:
        ...
        for d in data:
            if len(d['choices']) == 3:
                q = {'id': d['id'], 'answer': d['answer'],
                     'A': d['choices'][0]['content'], 'B': d['choices'][1]['content'], 'C': d['choices'][2]['content']}
                data_json = json.dumps(q, ensure_ascii=False)
            ...
            if len(d['choices']) == 5:
                q = {'id': d['id'], 'answer': d['answer'],
                     'A': d['choices'][0]['content'], 'B': d['choices'][1]['content'], 'C': d['choices'][2]['content'], 'D': d['choices'][3]['content'], 'E': d['choices'][4]['content']}
                data_json = json.dumps(q, ensure_ascii=False)
                file2.write(data_json)
                file2.write(',\n')
file2.close()
...
for b in f2:
    with open("001003判断/" + b, 'r', encoding="utf-8", errors="ignore") as load_f:
        ...
        for d in data:
            q = {'id': d['id'], 'answer': d['answer'],
                 'A': d['choices'][0]['content'], 'B': d['choices'][1]['content']}
            data_json = json.dumps(q, ensure_ascii=False)
